...
# 定义计算PPL的函数
def calculate_loss(model, tokenizer, context, target, device='cuda:3'):
    ...
    y_true = nn.functional.one_hot(target_ids, num_classes=logits.size(-1))
    y_pred = logits[None]
...
for i, item in enumerate(tqdm(json_data)):
    ...
    target = item["summary"]
    with torch.no_grad():
        loss = calculate_loss(quantized_model, tokenizer, context, target, device=device)
    total_loss += loss
    avg_loss = total_loss / (i + 1)
    ppl = torch.exp(torch.tensor(avg_loss).float()).cpu().detach().numpy()
    # 更新进度条下方的输出
    tqdm.write(f"当前平均Loss: {avg_loss:.4f}, 当前PPL: {ppl:.4f}")
